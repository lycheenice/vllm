...
class MooncakeConnector(KVConnectorBase_V1, SupportsHMA):
    ...
    def set_host_xfer_buffer_ops(self, copy_operation):
        """test4 CPU 中转:把运行器注入的 copy_blocks 转发到 worker。

        MooncakeConnector 带 SupportsHMA,kv_buffer_device=cpu 时运行器会调用本方法
        注入布局正确的块拷贝算子;worker 用它做 D2H/H2D。GPU 直传时 worker 忽略。
        """
        assert self.connector_worker is not None
        self.connector_worker.copy_blocks = copy_operation
...
class MooncakeConnectorWorker:
    ...
    def __init__(
        ...
        # --- test4 CPU 中转(host-staging)---------------------------------
        # kv_buffer_device=cpu 时:mooncake 注册/传输改用 host pinned buffer;
        # send 前 D2H(device->host),recv 完成后 H2D(host->device)。
        # copy_blocks 由模型运行器经 set_host_xfer_buffer_ops 注入(布局正确,复用
        # nixl 同一路径;MooncakeConnector 已带 SupportsHMA,runner 会注入)。
        self.kv_buffer_device: str = getattr(
            kv_transfer_config, "kv_buffer_device", "cuda"
        )
        self.use_host_buffer: bool = self.kv_buffer_device == "cpu"
        self.host_xfer_buffers: dict[str, torch.Tensor] = {}
        self.copy_blocks: Any | None = None
        # -----------------------------------------------------------------
    ...
    async def send_kv_to_decode(
        ...
        while wait_tasks:
            ...
            if src_ptrs:
                # test4 CPU 中转:传输从 host buffer 发出,故发送前把本次要传的
                # 本地 block 从 GPU D2H 到 host pinned buffer。仅 P 侧、use_host_buffer 时。
                if self.use_host_buffer:
                    for _d_req_id, _send_meta in ok_ready_reqs:
                        self._stage_host_blocks("d2h", _send_meta.local_block_ids)
    ...
    def _stage_host_blocks(
        self, direction: str, block_ids_by_group: list[list[int]]
    ) -> None:
        """test4 CPU 中转的 D2H/H2D 搬运。

        复用模型运行器经 set_host_xfer_buffer_ops 注入的 copy_blocks(与 nixl 同一路径,
        自动处理 KV 布局/分块)。block_ids_by_group 为按 KV 组分的本地逻辑块;此处展平
        (单组常见;多组共享同一块空间的情形由 copy_blocks 按索引处理)。
        direction: 'd2h' = device->host(P 发送前);'h2d' = host->device(D 接收后)。
        """
        if not self.use_host_buffer or self.copy_blocks is None:
            return
        flat = [b for grp in block_ids_by_group for b in grp]
        if not flat:
            return
        if direction == "d2h":
            self.copy_blocks(
                self.device_kv_caches, self.host_xfer_buffers, flat, flat, "d2h"
            )
        else:
            self.copy_blocks(
                self.host_xfer_buffers, self.device_kv_caches, flat, flat, "h2d"
            )
    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor]):
        ...
        # test4 CPU 中转:注册对象从 GPU kv_caches 换成 host pinned 镜像。
        # 每层分配等形状 device="cpu" pinned 张量;后续 register 用 host 地址,
        # 传输自然经宿主 DRAM;device 端仍留在 self.device_kv_caches 供 D2H/H2D。
        if self.use_host_buffer:
            self.host_xfer_buffers = {
                ln: torch.empty_like(t, device="cpu", pin_memory=True)
                for ln, t in kv_caches.items()
            }
            reg_caches = self.host_xfer_buffers
            logger.info(
                "test4: kv_buffer_device=cpu, registering %d host pinned "
                "buffers for mooncake (KV staged through host DRAM).",
                len(reg_caches),
            )
        else:
            reg_caches = kv_caches

        for layer_name, cache_or_caches in reg_caches.items():
    ...
    def process_pulling_result(
        ...
        for req_id in ok_reqs:
            ...
            if pull_meta.pull_tasks_count == 0:
                # test4 CPU 中转:该 req 的 KV 已全部拉入 D 侧 host buffer,
                # 标记完成前 H2D 回显存。仅 D 侧、use_host_buffer 时。
                if self.use_host_buffer:
                    self._stage_host_blocks("h2d", pull_meta.local_block_ids)
