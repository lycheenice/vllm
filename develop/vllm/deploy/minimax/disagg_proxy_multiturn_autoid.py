    ...
    if not conversation_id:
        ...
        for msg in messages:
            if msg.get("role") == "user":
                ...
                break
        if first_content:
            conversation_id = "auto_" + hashlib.sha256(
                first_content[:512].encode()).hexdigest()[:16]
            logger.info(
                "[%s] Auto-generated conversation_id=%s from first user msg",
                request_id, conversation_id,
            )
        else:
            logger.warning(
                "[%s] No conversation_id and no user message — KV cache "
                "reuse disabled for this request.",
                request_id,
            )
