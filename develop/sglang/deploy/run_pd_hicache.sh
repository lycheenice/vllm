#!/usr/bin/env bash
# Launch sglang PD with P1.1 hicache (L2 ratio=10 on both prefill+decode)
set -uo pipefail
...
PF_SCRIPT=start-prefill-hicache.sh
DC_SCRIPT=start-decode-hicache.sh
...
start)
  ...
  echo "PD-hicache up: router :8000"
  ;;
