#!/usr/bin/env bash
# Launch sglang PD P1.3: chunked_prefill 8192 + nocb router
set -uo pipefail
...
start)
  ...
  docker run -d --name pd-prefill --network host --gpus all -e CUDA_VISIBLE_DEVICES=0,1,2,3 \
    ...
    -v "$MODEL_VOL" -v "$SCRIPT_DIR/start-prefill-cp8k.sh:/start.sh" -v "$SCRIPT_DIR/logs/prefill:/root" --restart unless-stopped "$IMG" /start.sh
  ...
  docker run -d --name pd-decode --network host --gpus all -e CUDA_VISIBLE_DEVICES=4,5,6,7 \
    ...
    --cap-add SYS_NICE --cap-add IPC_LOCK --security-opt seccomp=unconfined --security-opt apparmor=unconfined --shm-size 32gb \
    -v "$MODEL_VOL" -v "$SCRIPT_DIR/start-decode-cp8k.sh:/start.sh" -v "$SCRIPT_DIR/logs/decode:/root" --restart unless-stopped "$IMG" /start.sh
  ...
  echo "PD-cp8k up: router :8000 (chunked_prefill=8192, nocb)"
  ;;
