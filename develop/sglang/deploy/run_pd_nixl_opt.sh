#!/usr/bin/env bash
# Launch optimized sglang PD (NIXL + disaggregation queue/thread + decode radix)
set -uo pipefail
...
wait_health() { local port="$1" name="$2" max="${3:-360}"; for i in $(seq 1 "$max"); do curl -sf "http://127.0.0.1:$port/health" >/dev/null 2>&1 && { echo "[$(date +%H:%M:%S)] $name healthy"; return 0; }; sleep 5; done; echo "ERR $name"; return 1; }
case "${1:-start}" in
start)
  ...
  wait_health 8001 prefill 360 || exit 1
  ...
  wait_health 8002 decode 360 || exit 1
  docker run -d --name pd-router --network host -v "$SCRIPT_DIR/start-router.sh:/start.sh" --restart unless-stopped "$IMG" /start.sh
  echo "PD-opt up: router :8000"
  ;;
stop) docker rm -f pd-router pd-decode pd-prefill 2>/dev/null; echo stopped;;
